...
async function setup(testOk = true, afterActivate: (id: string) => void = () => {}) {
...
describe('handleWebhook', () => {
  it('rejects a bad signature AND logs a REDACTED payload', async () => {
    ...
    const row = store.db.prepare('SELECT * FROM webhook_log ORDER BY id DESC LIMIT 1').get() as { payload_json: string; signature_ok: number };
  ...
  it('activates: install active, secret encrypted, provisioning run, afterActivate called', async () => {
    const after = vi.fn();
    const { repos, deps } = await setup(true, after);
    ...
    expect((await repos.installs.find('1'))?.status).toBe('active');
    expect((await loadConfigs(repos.state, '1', deps.integration.configSchema)).hiboutik_api_key).toBe('SECRETKEY');
    expect(after).toHaveBeenCalledWith('1');
    const prov = JSON.parse((await repos.state.get('1', '__provisioning')) ?? '{}') as { sourceIds: Record<string, number> };
  ...
  it('anti-replay: a signed webhook replayed verbatim is processed only once', async () => {
    const after = vi.fn();
    const { deps } = await setup(true, after);
  ...
  it('activates: provisioning FULLY failed -> success:false (inactive)', async () => {
    ...
    expect((await repos.installs.find('1'))?.status).toBe('inactive');
  });
  ...
  it('activates with the REAL NestJS payload (opaque installation_id, no legacy fields)', async () => {
    ...
    const res = await handleWebhook(body, headers, deps);
    expect(res.body.success).toBe(true);
    expect((await repos.installs.find('inst_opaque_abc123'))?.status).toBe('active');
  ...
  it('prefers installation_id over the legacy id_shop_integration when both are present', async () => {
    ...
    const res = await handleWebhook(body, headers, deps);
    expect(res.body.success).toBe(true);
    expect((await repos.installs.find('inst_opaque_xyz'))?.status).toBe('inactive');
    expect(await repos.installs.find('42')).toBeUndefined();
  });
  ...
  it('deactivation then uninstall change the status', async () => {
    ...
    expect((await repos.installs.find('1'))?.status).toBe('uninstalled');
  });
});

describe('handleTestConnection', () => {
  it('validates the credentials', async () => {
    const { deps } = await setup(true);
